...
{
    self.compColor = 
        [NSColor colorWithDeviceRed: (self.compRed / 255.0)
                              green: (self.compGreen / 255.0)
                               blue: (self.compBlue / 255.0)
                              alpha: 1.0];
    self.attColor = 
        [NSColor colorWithDeviceRed: (self.attRed / 255.0)
...
- (void) recalculateFrame
{
    ...
    oldFrame.size.width = 2 * GRAPH_PADDING + 
                        cellCount * (2 * CELL_PADDING + self.cellWidth);
...
- (void) drawRect: (NSRect) dirtyRect
{
    CGFloat     maxBarHeight = 
                    self.bounds.size.height - 2 * GRAPH_PADDING;
    NSRect      backgroundRect = self.bounds;
    [[NSColor lightGrayColor] setFill];
    [NSBezierPath fillRect: backgroundRect];
    
    backgroundRect = NSInsetRect(backgroundRect,
    ...
    for (NSUInteger index = 0; index < self.gameCount; index++) {
        Game *      game = [self.delegate passCompletion: self
                                             gameAtIndex: index];
        NSUInteger  stat;

        NSRect  barRect;
        barRect.origin.x = GRAPH_PADDING + 
                            index * (CELL_PADDING + self.cellWidth);
        barRect.origin.y = GRAPH_PADDING;
        barRect.size.width = self.cellWidth;
        stat = MIN(game.attempts.unsignedIntegerValue,
                   MAX_ATTEMPTS);
        barRect.size.height = stat * pixPerAttempt;
        [self.attColor setFill];
        [NSBezierPath fillRect: barRect];
        
        stat = MIN(game.completions.unsignedIntegerValue,
                   MAX_ATTEMPTS);
        barRect.size.height = stat * pixPerAttempt;
